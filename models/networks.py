...
def define_G(input_nc, output_nc, ngf, netG, norm='batch', use_dropout=False, init_type='normal', init_gain=0.02, gpu_ids=[], which = 'A'):
    """Create a generator
...
def define_D(input_nc, ndf, netD, n_layers_D=3, norm='batch', init_type='normal', init_gain=0.02, gpu_ids=[]):
    ...
    elif netD == 'glass':  # default PatchGAN classifier
        net = NLayerDiscriminator_2(input_nc, ndf, n_layers=3, norm_layer=norm_layer)
...
class NLayerDiscriminator_2(nn.Module):
    ...
    def __init__(self, input_nc, ndf=64, n_layers=3, norm_layer=nn.BatchNorm2d):
        ...
        sequence += [nn.Conv2d(ndf * nf_mult, 2, kernel_size=kw, stride=1, padding=padw),
                     nn.AdaptiveAvgPool2d(1)]  # output 1 channel prediction map
        self.model = nn.Sequential(*sequence)
